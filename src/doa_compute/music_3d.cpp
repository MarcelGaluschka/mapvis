...
MUSIC_3D::MUSIC_3D(int samplerate, uint16_t BUFFER_SIZE, int angles_x, int angles_y) :
    DOA_EST::DOA_EST()
{
    ...
    std::vector<double> temp_c_matrix;
    temp_c_matrix.resize(BUFFER_SIZE,0);
    c_matrix.resize(BUFFER_SIZE, temp_c_matrix);
...
std::array<double,3> MUSIC_3D::compute(std::array<int16_t*,8> *buffers)
{
    std::complex<double> imaginary(0.0,1.0);
    ...
    for (int i= 0; i < BUFFER_SIZE; i++)
    {
        for (int j = 0; j < BUFFER_SIZE; j++)
        {
            ...
            {
                c_matrix.at(i).at(j) += buffers->at(k)[i] * buffers->at(k)[j];
            }
        }
    }
    ...
    Matrix<double, Dynamic, Dynamic> eigen_mat (BUFFER_SIZE, BUFFER_SIZE);
    for(int i = 0; i < BUFFER_SIZE; i ++)
    {
        for (int j = 0; j < BUFFER_SIZE; j++)
        {
            eigen_mat(i,j) = c_matrix.at(i).at(j);
        }
    }
    std::cout << eigen_mat.rows() << std::endl;
    std::cout << eigen_mat(1,1) << " " << c_matrix.at(1).at(1) << std::endl;
    EigenSolver<MatrixXd> solver (eigen_mat);
    std::cout << "solver" << std::endl;
    //VectorXd values = solver.eigenvalues();
    solver.eigenvectors();
    std::cout << "solved" << std::endl;
    //EigenSolver<std::vector<std::vector<double>>> eigensolver;
    ...
    for (int curr_angl_id = 0; curr_angl_id < angles_x; curr_angl_id++)
    {
        ...
        for (int curr_angl_y_id = 0; curr_angl_y_id < angles_y; curr_angl_y_id++)
        {
            ...
            for (int i = 0; i < num_mics; i++)
            {
                ...
                inc_matrix2.at(i) = inc_matrix.at(i) * samplerate;   // ni
            }
